...
void DeepSearch1(int p_start, int tmp){
    ...
    for(int i=0;i<map_record[p_start][0].size();i++){
		...
		if(index2<=tmp || index2==p_start || index1 == index2)  continue;
		...
		for(int k=0;k<map_record[index2][0].size();k++){
			if(map_record[index2][0][k] <= tmp || map_record[index2][0][k] == p_start || map_record[index2][0][k] == index1 || map_record[index2][0][k]  == index2) continue;
...
void DeepSearch2(int p_start){
    ...
    for(int i=0;i<inverse_record[p_start][0].size();i++){
		...
		for(int j=0;j<inverse_record[index1][0].size();j++){
			...
			if(index2<=p_start || index2 == index1) continue;
			...
			for(int k=0;k<inverse_record[index2][0].size();k++){
				if( inverse_record[index2][0][k] <= p_start ||inverse_record[index2][0][k] == index1 || inverse_record[index2][0][k]== index2 ) continue;
